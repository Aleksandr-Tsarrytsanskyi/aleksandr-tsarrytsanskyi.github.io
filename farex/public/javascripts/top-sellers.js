...
async function getProducts() {
  try {
    ...
    if((productsData.length > COUNT_SHOW_CARDS_CLICK) && btnShowCards.classList.contains("none")) {
...
function renderStartPage(data) {
  ...
  let hit = data.filter(product => product.top == "hit")
  console.log(hit);
  
  const arrCards = hit.slice(0, COUNT_SHOW_CARDS_CLICK)
...
function sliceArrCards() {
  ...
  let hit = productsData.filter(product => product.top == "hit")
  countClickBtnShowCards++;
  const countShowCards = COUNT_SHOW_CARDS_CLICK * countClickBtnShowCards;
  const addCards = hit.slice(shownCardS, countShowCards);
